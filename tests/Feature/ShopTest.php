...
class ShopTest extends TestCase
{
    ...
    function a_player_can_unlock_characters_by_spending_chevron()
    {
        ...
        $id = $this->postJson(route('character.unlock'), ['gateway' => 'chevron', 'name' => 'Alyssa', 'player_id' => $player->id])
            ->assertSuccessful()
            ->json()['id'];
    ...
    function a_player_can_unlock_characters_by_real_money()
    {
        ...
        $id = $this->postJson(route('character.unlock'), ['name' => 'Alyssa', 'player_id' => $player->id])
            ->assertSuccessful()
            ->json()['id'];
    ...
    public function unlocking_characters_fails_if_player_id_is_not_specified()
    {
        ...
        $this->postJson(route('character.unlock'), ['id' => $template->id, 'gateway' => 'chevron'])
    ...
    function a_player_can_purchase_products_for_real_money()
    {
        ...
        $id = $this->postJson(route('product.purchase', $boost), ['player_id' => $player->id])
            ->assertSuccessful()
            ->json()['id'];
    ...
    public function virtual_products_require_a_player_id()
    {
        ...
        $this->postJson(route('product.purchase', $boost))
            ->assertJsonValidationErrors('player_id');
    }
